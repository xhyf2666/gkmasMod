...
public class UmeBecomeIdolPower extends AbstractPower {
    private static final String CLASSNAME = UmeBecomeIdolPower.class.getSimpleName();
    // 能力的ID
    ...
    public UmeBecomeIdolPower(AbstractCreature owner) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        ...
        if(count==2){
            for(AbstractMonster mo: AbstractDungeon.getMonsters().monsters){
                if(!mo.isDeadOrEscaped()&& !AbstractMonsterPatch.friendlyField.friendly.get(mo)){
                    addToBot(new ApplyPowerAction(mo,mo,new AttackTimePower(mo,1),1));
                }
            }
            count=0;
        }
