...
public class AchievementPower extends AbstractPower {
    ...
    public void atStartOfTurn() {
        if(!target.halfDead && !target.isDying && !target.isEscaping){
            addToBot(new ModifyDamageAction(target, new DamageInfo(this.owner, this.damage, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.SLASH_VERTICAL,new Achievement()));
        }
        else{
