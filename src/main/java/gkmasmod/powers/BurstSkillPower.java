...
public class BurstSkillPower extends AbstractPower {
    private static final String CLASSNAME = BurstSkillPower.class.getSimpleName();
    public static final String POWER_ID = NameHelper.makePath(CLASSNAME);
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(CLASSNAME);
    private static final String NAME = powerStrings.NAME;
    private static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;

    public BurstSkillPower(AbstractCreature owner, int Amount) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        ...
        if(this.owner instanceof AbstractCharBoss&&(!(card instanceof AbstractBossCard)))
            return;
        if (!card.purgeOnUse && (card.type == AbstractCard.CardType.SKILL||card.type == AbstractCard.CardType.POWER) && this.amount > 0) {
            flash();
