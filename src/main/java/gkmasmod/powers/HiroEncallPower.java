...
public class HiroEncallPower extends AbstractPower {
    private static final String CLASSNAME = HiroEncallPower.class.getSimpleName();
    ...
    private static int MAGIC = 50;

    public boolean flag1 = false;

    public HiroEncallPower(AbstractCreature owner,int amount) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        ...
        if(!flag1&&count<=0.5){
            ...
            for(AbstractMonster mo: AbstractDungeon.getMonsters().monsters){
                if(!mo.isDeadOrEscaped()&& !AbstractMonsterPatch.friendlyField.friendly.get(mo)){
                    addToBot(new ApplyPowerAction(mo,mo,new ReduceDamageReceive(mo,this.amount),this.amount));
                }
            }
        }
    }
