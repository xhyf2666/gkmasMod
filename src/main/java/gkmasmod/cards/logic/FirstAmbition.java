...
public class FirstAmbition extends GkmasCard {
    private static final String CLASSNAME = FirstAmbition.class.getSimpleName();
    ...
    public FirstAmbition() {
        ...
        this.customEffectList.add(CustomHelper.generateCustomEffectList(GoodImpressionCustom.growID, new int[]{1,1,2}, new int[]{40,40,60}, CustomHelper.CustomEffectType.GOOD_IMPRESSION_ADD));
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if(CustomHelper.hasCustom(this, EffectChangeCustom.growID)){
            addToBot(new ApplyPowerAction(p,p,new GiveYouSPPower(p)));
        }
        else{
            addToBot(new ApplyPowerAction(p,p,new GiveYouPower(p,this.magicNumber),this.magicNumber));
        }
        SoundHelper.playSound("gkmasModResource/audio/voice/skillcard/cidol_jsna_1_001_produce_skillcard_01.ogg");
    }

    @Override
    public AbstractCard makeCopy() {
        return new FirstAmbition();
    }
