...
public class DanceWithYou extends GkmasCard {
    private static final String CLASSNAME = DanceWithYou.class.getSimpleName();
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        int da = ThreeSizeHelper.getDa();
        count+= da/this.secondMagicNumber;
        if(count>this.thirdMagicNumber)
            count = this.thirdMagicNumber;
        addToBot(new ApplyPowerAction(p,p,new DanceWithYouPower(p,count),count));
    }

    @Override
    public AbstractCard makeCopy() {
        return new DanceWithYou();
    }
