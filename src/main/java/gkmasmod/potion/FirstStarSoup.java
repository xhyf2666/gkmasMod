...
import gkmasmod.powers.SelectFirstStarChaiPower;
import gkmasmod.utils.NameHelper;

import java.util.Iterator;

public class FirstStarSoup extends CustomPotion {
    private static final String CLASSNAME = FirstStarSoup.class.getSimpleName();
    ...
    public void use(AbstractCreature target) {
        ...
        if (AbstractDungeon.player != null && AbstractDungeon.player.hasRelic("SacredBark")) {
            ...
            addToBot(new ApplyPowerAction(AbstractDungeon.player,AbstractDungeon.player,new AnotherTurnPower(AbstractDungeon.player,1),1));
        }
        addToBot(new LoseHPAction(AbstractDungeon.player, AbstractDungeon.player, HP_LOST));
        addToBot(new ApplyPowerAction(AbstractDungeon.player,AbstractDungeon.player,new DoubleDamageReceive(AbstractDungeon.player,MAGIC),MAGIC));
        addToBot(new ApplyPowerAction(AbstractDungeon.player,AbstractDungeon.player,new AnotherTurnPower(AbstractDungeon.player,1),1));
    ...
    public void initializeData() {
        if (AbstractDungeon.player != null && AbstractDungeon.player.hasRelic("SacredBark")) {
            this.description = String.format(potionStrings.DESCRIPTIONS[0],HP_LOST*2,MAGIC*2,2);
        }
        else{
            this.description = String.format(potionStrings.DESCRIPTIONS[0],HP_LOST,MAGIC,1);
        }
