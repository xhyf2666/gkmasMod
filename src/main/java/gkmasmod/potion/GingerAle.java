...
public class GingerAle extends CustomPotion {
    private static final String CLASSNAME = GingerAle.class.getSimpleName();
    ...
    public void use(AbstractCreature target) {
        ...
        addToBot(new ChangeStanceAction(ConcentrationStance.STANCE_ID));
        addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new FullPowerValue(AbstractDungeon.player, amount), amount));
    }

    public void initializeData() {
        this.potency = getPotency();
        if (AbstractDungeon.player != null && AbstractDungeon.player.hasRelic("SacredBark")) {
            this.description = String.format(potionStrings.DESCRIPTIONS[1], potency);
        }
        else{
