...
import gkmasmod.powers.SenburiSodaPower;
import gkmasmod.utils.NameHelper;

import java.util.Iterator;

public class SenburiSoda extends CustomPotion {
    private static final String CLASSNAME = SenburiSoda.class.getSimpleName();
    ...
    public void use(AbstractCreature target) {
        if (AbstractDungeon.player != null && AbstractDungeon.player.hasRelic("SacredBark")) {
            ...
            addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new DrawCardNextXTurnPower(AbstractDungeon.player, MAGIC2,MAGIC3*2), MAGIC2));
            addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new SenburiSodaPower(AbstractDungeon.player, MAGIC2), MAGIC2));
        }
        else{
            addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new DrawCardNextXTurnPower(AbstractDungeon.player, MAGIC2,MAGIC3), MAGIC2));
        }
        addToBot(new LoseHPAction(AbstractDungeon.player, AbstractDungeon.player, HP_LOST));
        addToBot(new DrawCardAction(AbstractDungeon.player, MAGIC));
        addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new SenburiSodaPower(AbstractDungeon.player, MAGIC2), MAGIC2));
    ...
    public void initializeData() {
        if (AbstractDungeon.player != null && AbstractDungeon.player.hasRelic("SacredBark")) {
            this.description = String.format(potionStrings.DESCRIPTIONS[0], HP_LOST*2, MAGIC*2, MAGIC2,10*2, MAGIC3*2);
        }
        else{
            this.description = String.format(potionStrings.DESCRIPTIONS[0], HP_LOST, MAGIC, MAGIC2,10, MAGIC3);
        }
        this.tips.clear();
