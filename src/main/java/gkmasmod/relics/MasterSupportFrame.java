...
public class MasterSupportFrame extends MasterRelic {

    private static final String CLASSNAME = MasterSupportFrame.class.getSimpleName();
    ...
    private static final int magic1 = 15;
    ...
    private static final int CARD_GROW = 2;
    ...
    public String getUpdatedDescription() {
        if(style == CommonEnum.IdolStyle.GOOD_TUNE)
            return String.format(this.DESCRIPTIONS[0],BLOCK,MAGIC,unit2,goodTuneString,1,magic1,magic2);
        else if(style == CommonEnum.IdolStyle.FOCUS)
            return String.format(this.DESCRIPTIONS[0],BLOCK,MAGIC,unit1,focusString,1,magic1,magic2);
        else if(style == CommonEnum.IdolStyle.GOOD_IMPRESSION)
            return String.format(this.DESCRIPTIONS[0],BLOCK,MAGIC2,unit2,goodImpressionString,1,magic1,magic2);
        else if(style == CommonEnum.IdolStyle.YARUKI)
            return String.format(this.DESCRIPTIONS[0],BLOCK,MAGIC,unit1,yarukiString,1,magic1,magic2);
        else if(style == CommonEnum.IdolStyle.FULL_POWER)
            return String.format(this.DESCRIPTIONS[1],FULL_POWER,1,magic1,magic2);
        else if(style == CommonEnum.IdolStyle.CONCENTRATION)
            return String.format(this.DESCRIPTIONS[2],CARD_GROW,1,magic1,magic2);
        else
            return String.format(this.DESCRIPTIONS[0],BLOCK,MAGIC,unit1,focusString,1,magic1,magic2);
    }
    ...
    public void atBattleStart() {
        ...
        if(style == CommonEnum.IdolStyle.GOOD_TUNE){
            ...
            addToBot(new ChangeStanceAction(PreservationStance.STANCE_ID));
            GrowHelper.growAll(DamageGrow.growID,CARD_GROW);
        }
        else{
            addToBot(new GainBlockAction(AbstractDungeon.player, AbstractDungeon.player, BLOCK));
            addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new StrengthPower(AbstractDungeon.player, MAGIC), MAGIC));
