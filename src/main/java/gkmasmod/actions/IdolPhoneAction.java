...
import gkmasmod.relics.IdolPhone;
...
public class IdolPhoneAction extends AbstractGameAction {
    ...
    public IdolPhoneAction() {
        this.actionType = ActionType.CARD_MANIPULATION;
        this.duration = Settings.ACTION_DUR_FAST;
        this.amount = 1;
    }

    public IdolPhoneAction(int showCardCount) {
        this.actionType = ActionType.CARD_MANIPULATION;
        this.duration = Settings.ACTION_DUR_FAST;
    ...
    public void update() {
        ...
        if (!this.retrieveCard) {
            if (AbstractDungeon.cardRewardScreen.discoveryCard != null) {
                ...
                GrowHelper.grow(disCard, ExhaustRemoveGrow.growID,1);
                GrowHelper.grow(disCard, EnergyGrow.growID,-1);
                disCard.current_x = -1000.0F *  Settings.xScale;
                disCard2.current_x = -1000.0F * Settings.xScale + AbstractCard.IMG_HEIGHT_S;
                try{
                    IdolPhone.playVoice(((GkmasCard)disCard).backGroundColor);
                }
                catch (Exception e){}
                if (this.amount == 1) {
    ...
    private ArrayList<AbstractCard> generateCardChoices() {
        ...
        ArrayList<AbstractCard> tmpPool = CardHelper.getAllIdolCards();
