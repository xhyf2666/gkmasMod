...
public class GoodTuneReturnAction extends AbstractGameAction {
    private DamageInfo info;
    private float rate;

    public GoodTuneReturnAction(AbstractCreature target, DamageInfo info, float rate) {
        this.info = info;
        this.rate = rate;
        ...
        if (this.shouldCancelAction()) {
            ...
            if (this.isDone) {
                ...
                if (this.target.lastDamageTaken > 0) {
                    ...
                    count = (int) (count*this.rate);
                    this.addToTop(new ApplyPowerAction(this.source,this.source,new GoodTune(this.source,count),count));
