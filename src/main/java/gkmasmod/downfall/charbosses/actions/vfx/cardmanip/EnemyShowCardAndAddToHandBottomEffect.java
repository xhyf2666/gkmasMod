package gkmasmod.downfall.charbosses.actions.vfx.cardmanip;
...
public class EnemyShowCardAndAddToHandBottomEffect extends AbstractGameEffect {
    ...
    public EnemyShowCardAndAddToHandBottomEffect(final AbstractCard card, final float offsetX, final float offsetY) {
        this.card = card;
    ...
    public EnemyShowCardAndAddToHandBottomEffect(final AbstractCard card) {
        this.card = card;
    ...
    private void playCardObtainSfx() {
        ...
        for (final AbstractGameEffect e : AbstractDungeon.effectList) {
            if (e instanceof EnemyShowCardAndAddToHandBottomEffect) {
                ++effectCount;
            }
        }
        if (effectCount < 2) {
    ...
    private void identifySpawnLocation(final float x, final float y) {
        ...
        switch (effectCount) {
            ...
            case 1: {
                this.card.target_x = Settings.WIDTH * 0.5f - EnemyShowCardAndAddToHandBottomEffect.PADDING - AbstractCard.IMG_WIDTH;
                break;
            }
            case 2: {
                this.card.target_x = Settings.WIDTH * 0.5f + EnemyShowCardAndAddToHandBottomEffect.PADDING + AbstractCard.IMG_WIDTH;
            ...
            case 3: {
                this.card.target_x = Settings.WIDTH * 0.5f - (EnemyShowCardAndAddToHandBottomEffect.PADDING + AbstractCard.IMG_WIDTH) * 2.0f;
                break;
            }
            case 4: {
                this.card.target_x = Settings.WIDTH * 0.5f + (EnemyShowCardAndAddToHandBottomEffect.PADDING + AbstractCard.IMG_WIDTH) * 2.0f;
